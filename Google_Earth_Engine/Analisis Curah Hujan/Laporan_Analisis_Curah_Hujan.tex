...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textsf{\color{headergray}\textbf{A PREPRINT} --- EVALUASI PRESIPITASI MULTI-SKALA SATELIT VS AWS IOT}}
...
% =============================================================================
\title{\vspace{-1.2cm}\textbf{\Large Analisis Curah Hujan Multi-Skala: Evaluasi Komparatif 8 Produk Presipitasi Satelit dan Reanalisis terhadap Pengamatan Sensor AWS IoT Jerukagung pada Resolusi Jam-Jaman (\textit{Hourly}) dan Harian (\textit{Daily})}}

\author[1]{\textbf{Evan Alif Widhyatma}\thanks{Penulis korespondensi: \texttt{alifwidhyatma@gmail.com}}}
\affil[1]{\small Program Studi Sains Data, Fakultas Ilmu Komputer, Universitas Putra Bangsa, Kebumen, Jawa Tengah, Indonesia}
...
\begin{abstract}
\noindent Validasi dan kuantifikasi ketidakpastian estimasi curah hujan dari produk satelit dan model reanalisis atmosfer terhadap observasi stasiun darat otomatis (\textit{Automatic Weather Station} / AWS IoT) merupakan tahapan fundamental dalam pemodelan hidrologi dan mitigasi bencana hidrometeorologi. Penelitian ini menyajikan evaluasi komparatif multi-skala antara dataset presipitasi harian Kabupaten Kebumen (\texttt{Data\_Curah\_Hujan\_Kebumen.csv}) yang mencakup 8 produk satelit/reanalisis (\texttt{CHIRPS\_RNL}, \texttt{CHIRPS\_SAT}, \texttt{CHIRPS\_FNL}, \texttt{GSMaP}, \texttt{IMERG}, \texttt{PERSIANN}, \texttt{ERA5}, dan \texttt{ERA5\_LAND}) serta Pos Hujan \texttt{OYA}, dan dataset resolusi jam-jaman (\textit{hourly}) terhadap stasiun darat AWS IoT Jerukagung sepanjang 563 hari valid ($13.512$ jam observasi sinkron, 1 Januari 2025 s.d. 17 Juli 2026). Evaluasi dilakukan secara komprehensif menggunakan 9 metrik statistik terpadu ($r$, $\rho$, $\text{RMSE}$, $\text{MAE}$, $\text{MBE}$, $\text{PBIAS}$, $\text{NSE}$, $\text{KGE}$, dan $\text{IOA}$) serta metrik kontingensi kejadian hujan ($\text{POD}$, $\text{FAR}$, $\text{CSI}$, $\text{HSS}$). Hasil studi membuktikan adanya fenomena \textit{lonjakan akurasi multi-temporal} (\textit{multi-scale accuracy jump}): pada skala 1-jam, presipitasi satelit memiliki korelasi moderat ($r = 0{,}414$ untuk IMERG dan $r = 0{,}313$ untuk GSMaP) akibat pergeseran waktu mikro (\textit{sub-hourly lag}) dan perbedaan jejak spasial (\textit{spatial footprint mismatch}); namun saat diagregasikan ke skala harian ($24\text{h}$), akurasi korelasi melonjak drastis hingga $r = \mathbf{0{,}619}$ ($\rho = \mathbf{0{,}672}$, $\text{KGE} = 0{,}468$) untuk \texttt{NASA GPM IMERG} dan $r = \mathbf{0{,}625}$ ($\rho = \mathbf{0{,}641}$, $\text{KGE} = 0{,}569$) untuk \texttt{CHIRPS\_SAT}. Analisis siklus diurnal 24-jam mengungkap bahwa hujan konvektif di Kebumen terkonsentrasi sangat kuat pada sore hari (15:00--18:00 WIB) yang bertepatan dengan penurunan suhu permukaan pasca-insolasi maksimum dan osilasi semidiurnal tekanan udara.
\end{abstract}

\vspace{0.2cm}
\noindent\textbf{\textit{Keywords:}} Analisis Curah Hujan Multi-Skala, AWS IoT Jerukagung, NASA GPM IMERG, JAXA GSMaP, CHIRPS, ECMWF ERA5, Siklus Diurnal, Evaluasi Multimetrik.
...
\midrule
\texttt{Pos Hujan OYA}      & \textbf{0.759} & \textbf{0.724} & \textbf{9.18}  & \textbf{4.89} & -1.53 & -18.1\% & \textbf{+0.549} & \textbf{0.676} & \textbf{0.863} \\
...
\begin{figure}[htbp]
    ...
    \caption{Diagram Pencar Kepadatan Logaritmik (\textit{Hexbin Density Scatter}) 8 Produk Presipitasi Satelit \& Reanalisis Kebumen vs AWS IoT Harian.}
...
\texttt{Pos Hujan OYA}  & 0.666 & \textbf{0.759} & \textbf{+14.0\%} & 0.463 & \textbf{0.724} & 0.32 mm & 4.89 mm & \textbf{0.676} \\
\bottomrule
\end{tabular}%
}
\end{table}
